 ...
 */
function normalizeAssignedContacts(assignedContacts) {
  if (Array.isArray(assignedContacts)) return assignedContacts.filter(Boolean);
  if (assignedContacts && typeof assignedContacts === "object") {
    return Object.values(assignedContacts).filter(Boolean);
  }
  return [];
}
...
function renderSingleBadges(contacts) {
  return contacts
    .map((contact) => {
      const name = typeof contact === "string"
        ? contact
        : String(contact?.name || "Unknown");
      let initials = getTaskInitials(name);
      let color = typeof contact === "object" && contact?.color
        ? contact.color
        : "#2A3647";

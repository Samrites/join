...
function generateTaskDetailHTML(
  ...
  const creatorType = creator.isExternal ? "Extern" : "Member";
  ...
  const aiNotice = isAiGeneratedTask(todo)
    ? '<span class="ai-ticket-marker ai-generated-badge-detail"><img class="ai-ticket-icon" src="../assets/icons/ai-generated-wand.png" alt="" aria-hidden="true"><span>AI-generated ticket</span></span>'
    : "";
  const aiHeaderClass = isAiGeneratedTask(todo)
    ? "task-dialog-header--ai"
    : "task-dialog-header--standard";
  const creatorAction = creator.isExternal && isValidEmailAddress(creator.email)
    ? `<a class="creator-action creator-email" href="mailto:${escapeHTML(creator.email)}" aria-label="Send an email to ${escapeHTML(creator.email)}"><img src="../assets/icons/mail.svg" alt="" aria-hidden="true"><span>E-mail</span></a>`
    : !creator.isExternal
      ? '<a class="creator-action creator-profile" href="contacts.html" aria-label="Open member profile"><img src="../assets/icons/person.svg" alt="" aria-hidden="true"><span>Profil</span></a>'
      : "";
  const creatorIcon = creator.isExternal
    ? "../assets/icons/external-globe.png"
    : "../assets/icons/person.svg";
  const creatorModeClass = creator.isExternal
    ? "content-wrapper-task-dialog--external"
    : "content-wrapper-task-dialog--internal";

  return `
      <div id="content-wrapper--task-dialog" class="content-wrapper-task-dialog content-wrapper-task-dialog--detailed ${creatorModeClass} disabled-ui">
          <div class="task-dialog-header ${aiHeaderClass}">
          ...
          <div class="dialog-creator">
            <p class="dialog-text-label">Creator:</p>
            <div class="creator-identity">
              <span class="creator-type ${creatorClasses}"><img src="${creatorIcon}" alt="" aria-hidden="true">${escapeHTML(creatorType)}</span>
